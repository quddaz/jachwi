...
- [x] 체크 단계 열거형 구현: `ONLINE_PHONE`, `ON_SITE`, `PRE_CONTRACT`
- [x] 항목 유형 열거형 구현: `CORE`, `OPTIONAL`
- [x] 활성 시스템 체크 항목 공개 조회 구현
- [x] 단계 필수 필터 구현
- [x] 유형과 검색어 필터 구현
- [x] 핵심 항목 우선 정렬 구현
- [x] 비활성 항목의 신규 선택 차단
- [ ] 시스템 체크 항목 조회 페이지네이션 구현
...
- [x] `GET /api/check-items`

### 7. 사용자 체크리스트

- [x] 체크리스트 생성 구현
- [x] 이름 trim 및 1~50자 검증
- [x] 생성 시 같은 단계의 활성 핵심 항목 자동 포함
- [x] 같은 단계의 선택 항목 추가 구현
- [x] 다른 단계 항목 추가 차단
- [x] 사용자 직접 질문 생성 차단
- [x] 생성 시 자동 포함된 핵심 항목의 이후 제거 허용
- [x] 항목 최소 1개, 최대 100개 검증
- [x] 동일 시스템 항목 중복 방지
- [x] 항목 표시 순서 저장 및 변경 구현
- [x] 체크리스트 목록·상세 조회 구현
- [x] 생성 후 단계 변경 차단
- [x] 이름과 전체 항목 순서의 전체 교체 구현
- [x] 비활성 항목은 기존 체크리스트에서만 유지·재정렬 가능하도록 처리
- [x] `deleted_at`을 이용한 소프트 삭제 구현
- [x] 삭제된 체크리스트를 조회·수정·신규 적용 대상에서 제외
- [x] 체크리스트 삭제 후 기존 매물 체크 스냅샷 유지

구현 API:

- [x] `POST /api/checklists`
- [x] `GET /api/checklists`
- [x] `GET /api/checklists/{checklistId}`
- [x] `PUT /api/checklists/{checklistId}`
- [x] `DELETE /api/checklists/{checklistId}`
...
- [ ] 생성 시 자동 포함된 핵심 항목을 수정에서 제거할 수 있다.
...
- `V2__manage_system_check_item_timestamps.sql`: 시스템 체크 항목 생성·수정 시각을 DB에서 자동 관리
- `V3__manage_all_audit_timestamps.sql`: 모든 테이블의 생성·수정 시각을 DB 기본값과 자동 갱신으로 관리
